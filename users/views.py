...
#imports from the API Rancher script
...
def lamp_sftp_create_deployment(hostname, username):
    url = "https://172.26.51.0/k8s/clusters/c-kmmv8/v1/apps.deployments"
...
def hosting(request):
    ...
    if request.method == 'POST':
        ...
        if hostname == "":
            messages.error(request, "Please fill in a hostname")
            return render(request, 'users/hosting.html')  
        if username == "":
            messages.error(request, "Please fill in a username")
            return render(request, 'users/hosting.html')
        if password == "":
            messages.error(request, "Please fill in a password")
            return render(request, 'users/hosting.html')
        if UserChoice.objects.filter(hostname=hostname).exists():
            messages.error(request, f"Hostname {hostname.upper()} has already been taken.")
        else:
                                                                        ...
                                                                        if status_lamp_webserver_ingress != 201:
                                                                            ...
                                                                            if status_lamp_sftp_service[0] != 201:
                                                                                ...
                                                                                if status_lamp_sftp_configmap != 201:
                                                                                    ...
                                                                                    status_lamp_sftp_deployment = lamp_sftp_create_deployment(hostname, username)
                                                                                    if status_lamp_sftp_deployment != 201:
                                                                                        creating_deployment = False
                                                                                    else:
                                                                                        port = str(status_lamp_sftp_service[1])
                                                                                        UserChoice.objects.create(option=option, username=username, hostname=hostname, password=password)
                                                                                        messages.success(request,f"{hostname.upper()} will be available in 10 minutes @ www.{hostname}.motoyohosting.uk | phpmyadmin.{hostname}.motoyohosting.uk | sftp://{hostname}.motoyohosting.uk port --> {port}")
                                                                                        return render(request, 'users/hosting.html')
                                            ...
                                            if status_wordpress_web_service != 201:
                                                ...
                                                if status_wordpress_web_deployment != 201:
                                                    ...
                                                    if status_wordpress_web_ingress != 201:
                                                        ...
                                                        UserChoice.objects.create(option=option, username=username, hostname=hostname, password=password)
                                                        messages.success(request,f"{hostname.upper()} will be available in 10 minutes @ https://www.{hostname}.motoyohosting.uk")
                                                        return render(request, 'users/hosting.html')
                                                                 
            status_delete_namespace = delete_namespace(hostname)
            if status_delete_namespace == 201:
                messages.error(request, f"Deploying {hostname} has failed, please try again in 5 minutes.")
    return render(request, 'users/hosting.html')
